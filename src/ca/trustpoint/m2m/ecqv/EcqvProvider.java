...
public class EcqvProvider {
	 ...
	 *            Optional. Algorithm parameters. (not currently used)
	 */
	public EcqvProvider(String algorithmOid, byte[] parameters) throws IllegalArgumentException,
			UnsupportedOperationException, NoSuchAlgorithmException, NoSuchProviderException {
...
//			}
			...
			// from H(Certu)
			System.out.println("Certu = U||Pu = " + byte2HexStr(identifyingInfo) + byte2HexStr(reconstructionPoint));
		...
		System.out.println("return : Pu & r");
	...
	public PrivateKey genNewKey1(PrivateKey t, PublicKey T, PublicKey Qu, PrivateKey du, byte[] cert,
			byte[] reconstructionPoint) throws IOException {
		BigInteger n = curveParameters.getN();
	...
	public PublicKey verifyNewKey1(byte[] cert, PublicKey T, PublicKey Qca, byte[] reconstructionPoint)
			throws IOException {
		BigInteger n = curveParameters.getN();
		BigInteger be, bnewe;
		for (byte b : cert) {
	...
	public BigInteger genNewKey2(PrivateKey t, PublicKey T, PublicKey Qu, PrivateKey du) throws IOException {
		BigInteger n = curveParameters.getN();
...
//		System.out.println("e : " + e);
	...
	public Boolean verifyNewKey2(BigInteger z, byte[] cert, PublicKey T, PublicKey Qca, byte[] reconstructionPoint)
			throws IOException {
		BigInteger n = curveParameters.getN();
...
//			caKeyDefinition.setAlgorithm(M2mSignatureAlgorithmOids.ECQV_SHA256_SECP256K1);
	...
	public static void setFullCertData(byte[] fullCertData) {
		EcqvProvider.fullCertData = fullCertData;
	}
